...
1. Добавить VLESS WS inbound в 3x-ui
2. Обновить конфиг Caddy (добавить vpn.whitevpn.tech routing)
3. Добавить A-запись `vpn.whitevpn.tech` в CF с orange cloud
4. Включить gRPC в CF dashboard
5. Проверить: gRPC, xHTTP, WS через `vpn.whitevpn.tech:443`
6. Обновить конфиги в 3x-ui → subscription автообновляется у клиентов
7. Убедиться, что клиенты переключились → выключить лишние порты на relay
